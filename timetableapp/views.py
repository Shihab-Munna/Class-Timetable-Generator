...
def CourseView(request):
    ...
    if request.method == 'POST':
        ...
        if course.is_valid():
            ...
            messages.success(
                request, 'Course already exists or you have added wrong attributes.')
...
def ProfessorView(request):
    ...
    if request.method == 'POST':
        ...
        if professor.is_valid():
            ...
            messages.success(
                request, 'Professor already exists or you have added wrong attributes.')
    return render(request, 'timetableapp/AddProfessor.html', context)


def GetProfessorView(request):
    professor = ProfessorForm()
    professors = Professor.objects.all()
    professorList = []
    for professor in professors:
        professorList.append(professor.professor_name)
    print(professorList)

    return JsonResponse({'professor': professorList})
            ...
            if len(sectionrooms) > 0:
                ...
                for i in range(len(sectionrooms)):
                    room = Classroom.objects.get(
                        classroom_id=sectionrooms[i].classroom_id)
                    ...
                    try:
                        ...
                        try:
                            ...
                            while j < courseLecs:
                                ...
                                if DupNum < workingHours + 5:
                                    if DupNum < 5:
                                        lecDay = random.randint(
                                            0, totalDays - 1)
                                        lecStartTime = random.randint(
                                            section.start_time, section.end_time - 1)
                                    ...
                                    if lecFlag:
                                        if lecStartTime <= section.end_time - lecDuration:
                                            ...
                                            while course.course_type != class_rooms[roomNum].TYPE or tot < totalRooms:
                                                roomNum = (
                                                    roomNum + 1) % totalRooms
                                                tot += 1
                                            activityFlag = True
                                            activityID = [
                                                section.week_day[lecDay]] * int(lecDuration)
                                    ...
                                    deleteActivities(id)
                                    messages.error(
                                        request, 'Solution does not exist.')
                                    DupNum += 1
                                    break
...
def TimeTableView(request, id):
    try:
        ...
        for x in range(0, len(time)):
            time_slot[x] = str(section.start_time + x) + \
                ':00 - ' + str(section.start_time + x + 1) + ':00'
...
def AddRoutine(request):
    ...
    courses = Course.objects.all()
    rooms = Classroom.objects.all()
    context = {'professors': professors, 'courses': courses, 'rooms': rooms}
    ...
    return render(request, 'timetableapp/AddRoutine.html', context)


@csrf_exempt
def ValidateRoutine(request):
    if request.method == 'POST':
        ...
        routines = Routine.objects.filter(
            semester_type__exact=semester_type, day__exact=day, time_slot__exact=time_slot, room__exact=room)
        if len(routines) > 0:
            return JsonResponse({'isValid': False, 'message': semester_type + " " + day + " "+time_slot + " room: " + room + " already assigned"})
        else:
            return JsonResponse({'isValid': True, 'message': "Routine is valid"})


@login_required(login_url='login')
def ViewAllRoutines(request):
    availableRoutines = Routine.objects.values('semester_type').distinct()
    print('availableRoutines>>>>>>>>>>>>>>>>>>>>>>>>>: ', availableRoutines)
    context = {'availableRoutines': availableRoutines}

    return render(request, 'timetableapp/AllRoutineView.html', context)


@login_required(login_url='login')
def PrintRoutine(request, semester_type):
    routines = Routine.objects.filter(semester_type__exact=semester_type)

    weekdays = ['Saturday', 'Sunday', 'Monday',
                'Tuesday', 'Wednesday', 'Thursday']
    timeSlots = ['09:00-10:25', '10:30-11:55',
                 '12:00-01:25', '01:30-02:25', '02:30-03:25']
    dayAndTime = []
    dayAndTimeRoutine = []
    finalDayAndTime = []
    for timeSlot in timeSlots:
        for weekday in weekdays:
            dayAndTime.append(weekday + "-" + timeSlot)
    for routine in routines:
        dayAndTimeRoutine.append(routine.day + "-" + routine.time_slot)
    for dayAndTimeStaticSingle in dayAndTime:
        i = 0
        check = False
        for dayAndTimeRoutineSingle in dayAndTimeRoutine:
            if dayAndTimeStaticSingle == dayAndTimeRoutineSingle:
                check = True
                finalDayAndTime.append(
                    routines[i].course_details + "-" + routines[i].room + " (" + routines[i].semester+")")
                break
            i += 1
        if check == False:
            finalDayAndTime.append("-----")
    context = {'routines': routines, 'finalDayAndTime': finalDayAndTime}
    return render(request, 'timetableapp/RoutinePrint.html', context)


# # @login_required(login_url='login')
# def PrintRoutineNew(request, semester_type):
#     routines = Routine.objects.filter(semester_type__exact=semester_type)
#     routines_list = []
#     for routine in routines:
#         r = {
#             'semester_type': routine.semester_type,
#             'day': routine.day,
#             'time_slot': routine.time_slot,
#             'course_details': routine.course_details,
#             'room': routine.room,
#             'semester': routine.semester
#         }
#         routines_list.append(r)

#     # response = serializers.serialize(
#     #     "json", routines_list)

#     routines_json = JsonResponse({"routines": routines_list})
#     print(routines_list)

#     context = {routines_json}
#     return render(request, 'timetableapp/RoutinePrintNew.html', context)
